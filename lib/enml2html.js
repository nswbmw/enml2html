...
module.exports = function enml2html (note) {
  ...
  enml = enml.slice(start).trim() // remove <?xml version="1.0"...enml2.dtd">
  ...
  $('en-note').each(function () {
    ...
    self.replaceWith(`
      <div class="enNote" ${genStyle(self, ['name', 'title', 'lang', 'xml:lang', 'dir'])}>
        ${self.html()}
      </div>
    `)
  })
